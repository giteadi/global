-- Migration script for Global Exim Traders Database
...
ADD COLUMN category VARCHAR(100) AFTER category_id,
ADD INDEX idx_category_name (category);

-- Update products with category names from categories table
UPDATE products p 
SET p.category = (SELECT c.name FROM categories c WHERE c.id = p.category_id)
